...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		<a href="javascript:;" onclick="member_add('添加用户','${pageContext.request.contextPath}/jsps/phone-add.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a>
		<a href="javascript:;" onclick="member_add('导入Excel文件','${pageContext.request.contextPath}/jsps/phone-excel.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 导入Excel文件</a>
		<a href="${pageContext.request.contextPath}/phone/excelWrite" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe644;</i> 导出Excel文件</a></span>
		<span class="r">共有数据：<strong>${pageBean.allRow}</strong> 条</span> </div>
	<div class="mt-20">
	<div>显示
		<select id="pageSize">
			<option value="10">10</option>
			<option value="${pageContext.request.contextPath}/phone/pageQuery?currentPage=1&pageSize=5">50</option>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>${phone.telephone}</td>
				<td>${phone.address}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="">${phone.bankAccount}</u></td>
				...
				<td class="td-manage"><a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> 
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','${pageContext.request.contextPath}/phone/update?id=${phone.telephone }','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="javascript:;" onclick="member_del(this,'${phone.telephone}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<div>
		...
		<div style="float: right">
			<a id="prePageA" href="${pageContext.request.contextPath}/phone/pageQuery?currentPage=${pageBean.currentPage-1}&pageSize=${pageBean.pageSize}" ><button id="prePage" class="btn btn-secondary-outline radius">上一页</button></a>
			<label id="currentPage" value="${pageBean.currentPage}">${pageBean.currentPage}</label>
			<a id="nextPageA" href="${pageContext.request.contextPath}/phone/pageQuery?currentPage=${pageBean.currentPage+1}&pageSize=${pageBean.pageSize}" ><button id="nextPage" class="btn btn-secondary-outline radius">下一页</button></a>
		</div>
	</div>
</div>
<!--_footer 作为公共模版分离出去-->
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${pageContext.request.contextPath}/phone/delete?id='+id,
			dataType: 'json',
			success: function(data){
...
function batchDelete(){
    ...
    for (var i = 0, len = option.length; i < len; i++) {  
     ...
     $.ajax({  
              type: "post",  
              url:'${pageContext.request.contextPath}/phone/batchDelete',  
              data : {  
              "checkedId":checkedId  
              },  
              dataType:"json",  
