...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		<a href="javascript:;" onclick="member_add('添加用户','${pageContext.request.contextPath}/jsps/account-add.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a>
		<a href="javascript:;" onclick="member_add('导入Excel文件','${pageContext.request.contextPath}/jsps/account-excel.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 导入Excel文件</a>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="150">开户行</th>
				<th width="100">预存金额</th>
				<th width="100">消费额</th>
				<th width="100">当月消费额</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${requestScope.pageBean.list}" var="account">
			<tr class="text-c">
				<td><input type="checkbox" value="{account.id}" name="" id="id"></td>
				<td>${account.id}</td>
				<td>${account.customer_id}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="">${account.bankAccount}</u></td>
				<td>${account.openBank}</td>
				<td>${account.readyAmount}</td>
				<td>${account.costAmount}</td>
				<td class="text-l">${account.costMonthAmount}</td>
				...
				<td class="td-manage"><a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> 
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','${pageContext.request.contextPath}/acc/update?id=${account.id }','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="javascript:;" onclick="member_del(this,'${account.id}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<div>
		...
		<div style="float: right">
			<a id="prePageA" href="${pageContext.request.contextPath}/acc/pageQuery?currentPage=${pageBean.currentPage-1}&pageSize=${pageBean.pageSize}" ><button id="prePage" class="btn btn-secondary-outline radius">上一页</button></a>
			<label id="currentPage" value="${pageBean.currentPage}">${pageBean.currentPage}</label>
			<a id="nextPageA" href="${pageContext.request.contextPath}/acc/pageQuery?currentPage=${pageBean.currentPage+1}&pageSize=${pageBean.pageSize}" ><button id="nextPage" class="btn btn-secondary-outline radius">下一页</button></a>
		</div>
	</div>
</div>
<!--_footer 作为公共模版分离出去-->
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${pageContext.request.contextPath}/acc/delete?id='+id,
			dataType: 'json',
			success: function(data){
...
function batchDelete(){
    ...
    for (var i = 0, len = option.length; i < len; i++) {  
     ...
     $.ajax({  
              type: "post",  
              url:'${pageContext.request.contextPath}/acc/batchDelete',  
              data : {  
              "checkedId":checkedId  
              },  
              dataType:"json",  
